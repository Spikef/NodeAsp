...
var http = module.exports = function(){
    ...
    if ( queryString ) this.url += '?' + queryString;

    this.headers = {};
    var ServerVariables = new Enumerator(Request.ServerVariables);
    for ( ; !ServerVariables.atEnd() ; ServerVariables.moveNext() ) {
        var item = ServerVariables.item();
        if ( /^HTTP_/.test(item) ) {
            var name = item.replace(/^HTTP_/, '').toLowerCase();
            this.headers[name] = Request.ServerVariables(item)();
        }
    }
